...
class MasterScheduleService(
    ...
    /**
     * Get bulk schedule with flat groups structure for mobile apps (Android/iOS)
     * Returns a simplified response with all groups flattened into a single array
     */
    suspend fun getBulkScheduleFlat(seasonId: String, matchdayNumber: Int): BulkScheduleResponseFlat {
        // Fetch all data
        val categories = categoryRepository.getBySeasonId(seasonId)
        ...
        // Flatten all groups from all categories
        val flatGroups = mutableListOf<BulkScheduleGroupFlat>()
        ...
        for (category in categories) {
            ...
            matchDayWithGroups?.dayGroups?.forEach { dayGroup ->
                flatGroups.add(
                    BulkScheduleGroupFlat(
                        id = dayGroup.id,
                        categoryId = category.id,
                        categoryName = category.name,
                        categoryColorHex = category.colorHex ?: "#000000",
                        groupNumber = dayGroup.groupNumber,
                        matchDate = dayGroup.matchDate,
                        timeSlot = dayGroup.timeSlot,
                        courtIndex = dayGroup.courtIndex,
                        players = dayGroup.players.map { player ->
                            BulkSchedulePlayerFlat(
                                id = player.id,
                                name = player.name
                            )
                        },
                        playedCount = dayGroup.completedRotations,
                        totalCount = dayGroup.totalRotations
                    )
                )
            }
        }

        // Convert categories to flat format
        val flatCategories = categories.map { category ->
            BulkScheduleCategoryFlat(
                id = category.id,
                name = category.name,
        ...
        return BulkScheduleResponseFlat(
            seasonId = seasonId,
            matchdayNumber = matchdayNumber,
