-- Migration: Support 20-player categories
-- This updates the calendar generation to support both 16 and 20 player categories

-- ============================================================================
-- STEP 1: Update generate_league_calendar to support 16 or 20 players
-- ============================================================================
...
DECLARE
    ...
    v_current_date text;
BEGIN
    -- Get player count and season info
    ...
    RETURN json_build_object(
        ...
        'semifinals', 1,
        'final', 1,
        'totalMatches', v_groups_per_matchday * 5 * 3 + 9, -- Regular matches + playoff matches
        'datesApplied', v_matchday_dates IS NOT NULL
    );
END;
$function$;

COMMENT ON FUNCTION generate_league_calendar(uuid) IS 'Generates complete league calendar for 16 or 20 player categories. Creates 5 regular matchdays (4 or 5 groups), semifinals, and final.';
